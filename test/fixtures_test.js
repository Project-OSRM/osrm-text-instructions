...
tape.test('verify existence/update fixtures', function(assert) {
  ...
  function checkOrWrite(step, p, options) {
    ...
    if (process.env.UPDATE) {
      ...
      // check for existance
      assert.ok(fs.existsSync(fileName), `verified existence of ${testName}`);
    }
  }
  ...
  ['modes', 'other', 'arrive_waypoint', 'arrive_waypoint_last']
    .concat(constants.types)
    .forEach(type => {
      ...
      switch (type) {
        ...
        case 'depart':
          ...
          constants.directions.forEach(direction => {
            direction.slice(1).forEach(bearing => {
              ...
              checkOrWrite(
                step,
                path.join(basePath, `${direction[0]}_${bearing}`)
              );
            });
          });
          break;
        case 'modes':
          ...
          checkOrWriteVariations(
            baseStep,
            path.join(basePath, 'ferry_turn_left')
          );
          ...
          checkOrWriteVariations(
            baseStep,
            path.join(basePath, 'ferry_fork_left')
          );
        ...
        case 'other':
          ...
          checkOrWrite(
            baseStep,
            path.join(basePath, 'way_name_ref_destinations')
          );
          baseStep = {
            maneuver: {
              type: 'turn',
          ...
          checkOrWrite(
            baseStep,
            path.join(basePath, 'way_name_ref_mapbox_hack_1')
          );
          baseStep = {
            maneuver: {
              type: 'turn',
          ...
          checkOrWrite(
            baseStep,
            path.join(basePath, 'way_name_ref_mapbox_hack_2')
          );
          baseStep = {
            maneuver: {
              type: 'turn',
          ...
          checkOrWrite(
            baseStep,
            path.join(basePath, 'way_name_ref_mapbox_hack_3')
          );
          constants.modifiers.forEach(modifier => {
            var step = {
              maneuver: {
            ...
            checkOrWrite(
              step,
              path.join(basePath, `motorway_ref_has_number_${modifier}`),
              {
                classes: ['motorway']
              }
            );
          ...
          constants.modifiers.forEach(modifier => {
            ...
            checkOrWrite(
              step,
              path.join(basePath, `motorway_ref_has_no_number_${modifier}`),
              {
                classes: ['motorway']
              }
            );
          ...
          constants.modifiers.forEach(modifier => {
            ...
            checkOrWrite(
              step,
              path.join(basePath, `way_name_class_ferry_${modifier}`),
              {
                classes: ['ferry']
              }
            );
        ...
        case 'turn':
          ...
          constants.modifiers.forEach(modifier => {
            ...
            checkOrWriteVariations(
              baseStep,
              path.join(basePath, underscorify(modifier))
            );
          });
          break;
        case 'rotary':
  ...
  function phraseForLanguages(phrase, options) {
    ...
    supportedCodes.forEach(k => {
      result[k] = compiler.tokenize(
        k,
        instructions[k][version].phrase[phrase],
        options
      );
    });
  ...
  function checkOrWritePhrase(basePath, phrase, options) {
    ...
    if (process.env.UPDATE) {
      ...
      // check for existance
      assert.ok(fs.existsSync(fileName), `verified existance of ${phrase}`);
    }
  }
